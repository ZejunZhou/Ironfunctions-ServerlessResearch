...
import (
	...
	"github.com/harlow/go-micro-services/worker/common"
	"github.com/harlow/go-micro-services/worker/config"
	"github.com/harlow/go-micro-services/worker/ipc"
	"github.com/harlow/go-micro-services/worker/protocol"
...
func (w *FuncWorker) doHandshake() error {
	...
	// // parse handshake response, get flags for nested calls
	...
	if w.isGrpcSrv {
		...
		if err != nil {
			return fmt.Errorf("GrpcNew failed: %v", err)
		}
		w.grpcHandler = handler
	} else {
		handler, err := w.factory.New(w, w.configEntry.FuncName)
		if err != nil {
			return fmt.Errorf("New handler failed: %v", err)
		}
		w.handler = handler
	}
	...
	if err != nil {
		return fmt.Errorf("FifoOpenForWrite failed: %v %v", err, w.clientId)
	}
	w.outputPipe = op
